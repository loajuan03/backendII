...
public class AddressController {
    ...
    public List<AddressResponse> listAddresses(
        ...
        return addressApplicationService.listCurrentUserAddresses(
    ...
    public ResponseEntity<AddressResponse> addAddress(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(addressApplicationService.addAddress(
                        currentSessionResolver.resolveCurrentToken(auth),
                        request));
    }
    ...
    public AddressResponse updateAddress(
            ...
            @Valid @RequestBody AddressUpsertRequest request) {
        return addressApplicationService.updateAddress(
                currentSessionResolver.resolveCurrentToken(auth),
                id,
                request);
    }
    ...
    public AddressResponse setDefaultAddress(
            @RequestHeader(value = HttpHeaders.AUTHORIZATION, required = false) String auth,
            @PathVariable Long id) {
        return addressApplicationService.setDefaultAddress(
                currentSessionResolver.resolveCurrentToken(auth),
                id);
    }
    ...
    public ResponseEntity<Void> deleteAddress(
            @RequestHeader(value = HttpHeaders.AUTHORIZATION, required = false) String auth,
            @PathVariable Long id) {
        addressApplicationService.deleteAddress(currentSessionResolver.resolveCurrentToken(auth), id);
